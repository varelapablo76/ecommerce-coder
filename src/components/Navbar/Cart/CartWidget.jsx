...
const CartWidget = () => {
  const { listaCarrito, emptyCart, valorTotal } = UsoCarritoContext();
  ...
  const realizarCompra = async (e) => {
    ...
    const stockCollectionFirebase = query(
      ...
      where(
        documentId(),
        "in",
        listaCarrito.map((producto) => producto.id)
      )
    );
      ...
      .then((resp) =>
        resp.docs.forEach((res) =>
          batch.update(res.ref, {
            stock:
              res.data().stock -
              listaCarrito.find((item) => item.id === res.id).cantidad,
          })
        )
      )
      .catch((err) => console.log(err))
